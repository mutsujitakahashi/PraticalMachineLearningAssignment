...
### rf
```{r rfLoad, , echo=FALSE, eval=TRUE, cache=TRUE}
load("2021-06-28.RData")
fit_rf <- fit
pred_rf <- predict(fit_rf, newdata=test)
```

```{r rf, echo=FALSE, eval=FALSE}
fit_rf <- train(classe ~ ., method="rf", data=train)
pred_rf <- predict(fit_rf, newdata=test)
```
```{r rf_result, echo=TRUE, cache=TRUE}
fit_rf$finalModel
confusionMatrix(data=pred_rf, reference=test$classe)
```
train:`r fit$results$Accuracy[1]`  
test:`r confusionMatrix(data=pred,reference=test$classe)$overall[1]`  
